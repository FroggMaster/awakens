...
$(function() {
    ...
    function buildMessage(message) {
        ...
        if (message.count){
            var pairNum = message.count.toString().match(/(\d)\1*$/)[0].length;
            var colorSet = "#"
            switch(pairNum) {
                case 1:
                    //do nothing
            ...
            if (colorSet == "#"){
                ...
                el.append($('<div id=spooky_msg_' + message.count + ' class="timestamp" style = "color:'+colorSet+'" title=' + message.count + '></div>').text(time.format(DATE_FORMAT) + ' '));
            }
        } else
...
var mouseY;
parser = {
    linkreg : /(https?:\/\/)?((([a-z\d]([a-z\d-]*[a-z\d])*)\.)+[a-z]{2,}|((\d{1,3}\.){3}\d{1,3}))(\:\d+)?(\/[-a-z\d%_.~+]*)*(\?[;&a-z\d%_.~+=-]*)?(\#[-a-z\d_]*)?/gi,
    ...
    multipleColor : function(str, mtch, rep) {
        var ct = 0;
        while (str.match(mtch) != null && ct++ < 9001)
            str = str.replace(mtch, rep);
        return str;
    },
    loadedFonts : {},
    ...
    parse : function(str) {
        ...
        //colors
        str = this.multipleColor(str, /&#35;&#35;([\da-f]{6})(((?!&).)+)/i, '<span style="background-color: #$1;">$2</span>');
        str = this.multipleColor(str, /&#35;&#35;([\da-f]{3})(((?!&).)+)/i, '<span style="background-color: #$1;">$2</span>');
        str = this.multipleColor(str,/&#35;([\da-f]{6})(((?!&).)+)/i, '<span style="color: #$1;">$2</span>');
        str = this.multipleColor(str, /&#35;([\da-f]{3})(((?!&).)+)/i, '<span style="color: #$1;">$2</span>');
        str = this.multipleColor(str, RegExp('&#35;&#35;(' + this.coloreg + ')(((?!&).)+)', 'i'), '<span style="background-color: $1;">$2</span>');
        str = this.multipleColor(str, RegExp('&#35;(' + this.coloreg + ')(((?!&).)+)', 'i'), '<span style="color: $1;">$2</span>');
        str = this.multipleColor(str, this.fontRegex, '<span style="font-family:\'$1\'">$2</span>');
        // filters
